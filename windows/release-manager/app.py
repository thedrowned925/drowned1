...
QLineEdit,QComboBox,QTextEdit,QPlainTextEdit{background:#101622;border:1px solid #263247;border-radius:10px;padding:9px}
...
class Manager(QMainWindow):
    def __init__(self):
        ...
        tabs=QTabWidget(); tabs.addTab(self._publish_tab(),"Yeni Yayın"); tabs.addTab(self._settings_tab(),"GitHub"); self.setCentralWidget(tabs)
    def _settings_tab(self):
        ...
        note=QLabel("Token yalnızca işletim sistemi keyring/Windows Credential Manager içinde saklanır; kaynak koda yazılmaz."); note.setWordWrap(True); note.setStyleSheet("color:#8d99aa"); v.addWidget(note); v.addStretch(); return w
    ...
    def publish(self):
        ...
        params={'token':self.token.text(),'owner':self.owner.text(),'repo':self.repo.text(),'branch':self.branch.text(),'source':self.source.text(),'title':self.game_title.text().strip(),'platform':self.platform.currentText(),'channel':self.channel.currentText(),'version':self.version.text().strip(),'description':self.description.toPlainText(),'artwork':{'hero':self.hero.path,'cover':self.cover.path,'logo':self.logo.path}}
    ...
    def on_done(self,tag): self.publish_button.setEnabled(True); self.progress.setValue(100); QMessageBox.information(self,"Yayınlandı",tag)
